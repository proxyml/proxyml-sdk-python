...
### Changed
- `to_challenger_upload()` now raises `ValueError` immediately if
  `challenger_data_fingerprint` and `champion_data_fingerprint` both end up
  present and don't match, via the new
  `proxyml_core.check_fingerprints()`/`FINGERPRINT_MISMATCH_MESSAGE` — the
  same check the upload endpoint itself performs. Catches a challenger and
  champion scored on different data (e.g. one side went through an extra
  `dropna()` or a different train/test split) locally, before a round-trip
  to the server, with the identical error message either side would give.
...
### Fixed
- `train_challenger()`'s SIMPLE and FLEXIBLE classifier rungs now explicitly
  set `scoring="accuracy"` and `use_legacy_attributes=False` on
  `LogisticRegressionCV`, matching the MODERATE rung and silencing sklearn
  `FutureWarning`s that fired on every classification fit.
- Requires `proxyml-core>=0.2.1`, which fixes `binarize_if_probabilities()`
  silently mis-binarizing NaN/infinite targets to `0` instead of raising.
